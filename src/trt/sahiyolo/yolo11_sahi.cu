...
bool Yolo11SahiModelImpl::load(const std::string &engine_file,
...
InferResult Yolo11SahiModelImpl::forwards(const std::vector<cv::Mat> &inputs, void *stream)
{
    ...
    if (infer_batch_size  != num_image)
    {
        if (isdynamic_model_)
        {
            printf("num image: %d, max_batch_size: %d\n", num_image, max_batch_size_);
...
static InferBase *loadraw(const std::string &engine_file,
                          ...
                          double slice_vertical_ratio)
{
    Yolo11SahiModelImpl *impl = new Yolo11SahiModelImpl();
...
std::shared_ptr<InferBase> load_yolo_11_sahi(const std::string &engine_file,
                                             const std::vector<std::string> &names,
                                             int gpu_id,
                                             float confidence_threshold,
                                             float nms_threshold,
                                             ...
                                             double slice_vertical_ratio)
{
    try
    {
        ...
        return std::shared_ptr<Yolo11SahiModelImpl>((Yolo11SahiModelImpl *)loadraw(engine_file,
                                                                                   names,
                                                                                   confidence_threshold,
                                                                                   nms_threshold,
                                                                                   gpu_id,
