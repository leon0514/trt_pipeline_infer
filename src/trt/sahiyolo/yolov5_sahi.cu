...
bool Yolov5SahiModelImpl::load(const std::string &engine_file,
...
InferResult Yolov5SahiModelImpl::forwards(const std::vector<cv::Mat> &inputs, void *stream)
{
    ...
    for (int ib = 0; ib < num_image; ++ib)
    {
        ...
        decode_kernel_invoker_v5(image_based_bbox_output,
                                 bbox_head_dims_[1],
                                 num_classes_,
                                 bbox_head_dims_[2],
...
static InferBase *loadraw(const std::string &engine_file,
                          ...
                          double slice_vertical_ratio)
{
    Yolov5SahiModelImpl *impl = new Yolov5SahiModelImpl();
...
std::shared_ptr<InferBase> load_yolo_v5_sahi(const std::string &engine_file,
                                             const std::vector<std::string> &names,
                                             int gpu_id,
                                             float confidence_threshold,
                                             float nms_threshold,
                                             ...
                                             double slice_vertical_ratio)
{
    try
    {
        ...
        return std::shared_ptr<Yolov5SahiModelImpl>((Yolov5SahiModelImpl *)loadraw(engine_file,
                                                                                   names,
                                                                                   confidence_threshold,
                                                                                   nms_threshold,
                                                                                   gpu_id,
