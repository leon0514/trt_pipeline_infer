...
static inline bool check_ffmpeg_retvalue(int e, const char *call, int iLine, const char *szFile)
{
    if (e < 0)
    {
        std::cout << "FFMPEGDemuxer error " << call << ", cdoe = " << e << " in file " << szFile << ":" << iLine
                  << std::endl;
        return false;
    }
    return true;
...
class FFmpegDemuxerImpl : public FFmpegDemuxer
{
  public:
    ...
    void get_extra_data(uint8_t **ppData, int *bytes) override
    {
        ...
        // av_bitstream_filter_filter(bsfc, m_fmtc->streams[m_iVideoStream]->codec, nullptr, ppData, bytes, *ppData,
        // *bytes, 0); av_bitstream_filter_close(bsfc);
  ...
  private:
    double r2d(AVRational r) const { return r.num == 0 || r.den == 0 ? 0. : (double)r.num / (double)r.den; }
    ...
    bool open(AVFormatContext *fmtc, int64_t timeScale = 1000 /*Hz*/)
    {
        ...
        m_bMp4H264 = m_eVideoCodec == AV_CODEC_ID_H264 && (!strcmp(fmtc->iformat->long_name, "QuickTime / MOV") ||
                                                           !strcmp(fmtc->iformat->long_name, "FLV (Flash Video)") ||
                                                           !strcmp(fmtc->iformat->long_name, "Matroska / WebM"));
        m_bMp4HEVC = m_eVideoCodec == AV_CODEC_ID_HEVC && (!strcmp(fmtc->iformat->long_name, "QuickTime / MOV") ||
                                                           !strcmp(fmtc->iformat->long_name, "FLV (Flash Video)") ||
                                                           !strcmp(fmtc->iformat->long_name, "Matroska / WebM"));

        m_bMp4MPEG4 = m_eVideoCodec == AV_CODEC_ID_MPEG4 && (!strcmp(fmtc->iformat->long_name, "QuickTime / MOV") ||
                                                             !strcmp(fmtc->iformat->long_name, "FLV (Flash Video)") ||
                                                             !strcmp(fmtc->iformat->long_name, "Matroska / WebM"));
    ...
    AVFormatContext *CreateFormatContext(shared_ptr<DataProvider> pDataProvider)
    {
        ...
        m_avioc =
            avio_alloc_context(avioc_buffer, avioc_buffer_size, 0, pDataProvider.get(), &ReadPacket, nullptr, nullptr);
        if (!m_avioc)
        {
            INFOE("FFmpeg error");
            return nullptr;
  ...
  private:
    ...
    AVPacket m_pkt, m_pktFiltered; /*!< AVPacket stores compressed data typically exported by demuxers and then passed
                                      as input to decoders */
