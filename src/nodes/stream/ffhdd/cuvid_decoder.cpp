...
void convert_nv12_to_bgr_invoker(
    const uint8_t *y, const uint8_t *uv, int width, int height, int linesize, uint8_t *dst_bgr, cudaStream_t stream);
...
{
  public:
    bool create(bool bUseDeviceFrame,
                int gpu_id,
                cudaVideoCodec eCodec,
                bool bLowLatency            = false,
                const CropRect *pCropRect   = nullptr,
                const ResizeDim *pResizeDim = nullptr,
                int max_cache               = -1,
                int maxWidth                = 0,
                int maxHeight               = 0,
                unsigned int clkRate        = 1000,
                bool output_bgr             = false)
    {
    ...
    virtual int device() override { return this->m_gpuID; }

    virtual bool is_gpu_frame() override { return this->m_bUseDeviceFrame; }
    ...
    int handleVideoSequence(CUVIDEOFORMAT *pVideoFormat)
    {
        ...
        if ((pVideoFormat->coded_width >> 4) * (pVideoFormat->coded_height >> 4) > decodecaps.nMaxMBCount)
        {
            ...
            errorString << std::endl
                        << "MBCount             : "
                        << (pVideoFormat->coded_width >> 4) * (pVideoFormat->coded_height >> 4) << std::endl
        ...
        if (m_eChromaFormat == cudaVideoChromaFormat_420)
            ...
            m_eOutputFormat = pVideoFormat->bit_depth_luma_minus8 ? cudaVideoSurfaceFormat_YUV444_16Bit
    ...
    int handlePictureDisplay(CUVIDPARSERDISPINFO *pDispInfo)
    {
        ...
        checkCudaDriver(cuvidMapVideoFrame(m_hDecoder,
        ...
        if (m_output_bgr)
        {
            if (m_pYUVFrame == 0)
            {
                checkCudaDriver(cuMemAlloc(&m_pYUVFrame,
                                           m_nWidth * (m_nLumaHeight + m_nChromaHeight * m_nNumChromaPlanes) * m_nBPP));
            }
            if (m_pBGRFrame == 0)
            {
            ...
            if (m_bUseDeviceFrame)
            {
                checkCudaDriver(cuMemcpyDtoDAsync((CUdeviceptr)pDecodedFrame,
                                                  m_pBGRFrame,
                                                  m_nWidth * m_nLumaHeight * 3,
                                                  m_cuvidStream));
            }
            else
            {
                checkCudaDriver(
                    cuMemcpyDtoHAsync(pDecodedFrame, m_pBGRFrame, m_nWidth * m_nLumaHeight * 3, m_cuvidStream));
            }
        }
        else
        {
    ...
    virtual ICUStream get_stream() override { return m_cuvidStream; }
...
std::shared_ptr<CUVIDDecoder> create_cuvid_decoder(bool bUseDeviceFrame,
                                                   IcudaVideoCodec eCodec,
                                                   int max_cache,
                                                   int gpu_id,
                                                   const CropRect *pCropRect,
                                                   const ResizeDim *pResizeDim,
                                                   bool output_bgr)
{

    shared_ptr<CUVIDDecoderImpl> instance(new CUVIDDecoderImpl());
    if (!instance->create(bUseDeviceFrame,
                          gpu_id,
                          (cudaVideoCodec)eCodec,
                          false,
                          pCropRect,
